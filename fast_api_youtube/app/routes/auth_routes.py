...
from app.bank.schemas import UsuarioSchema, LoginSchema
...
async def criar_conta(usuario_schema: UsuarioSchema, session=Depends(pegar_sessao)):
    try:
        ...
        if usuario:
            raise HTTPException(
                status_code=400,
                detail=f"Usuário com email {usuario_schema.email} já existe",
            )
        else:
...
async def login(login_schema: LoginSchema, session=Depends(pegar_sessao)):
    ...
    if not usuario:
        raise HTTPException(status_code=400, detail=f"Usuário não encontrado ou senha incorreta")
    else:
...
async def login_form(dados_form: OAuth2PasswordRequestForm = Depends(), session=Depends(pegar_sessao)):
